import { BAD_REQUEST, CREATED, OK, UNAUTHORIZED, NOT_FOUND } from "../constants/http";
import { createProductCategory, getProductCategories, getProductCategoryById, updateCategory, deleteCategory } from "../services/productCategory.service";
...
export const getProductCategoryHandler = catchErrors(async (req, res) => {
    ...
    appAssert(productCategory, NOT_FOUND, `Product category with id ${id} not found or not owned by you`);
...
export const updateCategoryHandler = catchErrors(async (req, res) => {
    ...
    appAssert(existing, NOT_FOUND, `Product category with id ${categoryId} not found or not owned by you`);
