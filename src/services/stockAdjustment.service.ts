...
export const createStockAdjustment = async (
    data: { productId: string; quantityChange: number; reason: string; note?: string },
    userId: string
) => {
    ...
    if (newStock < 0) throw new AppError(BAD_REQUEST, "Stock cannot go negative");
...
export const getStockAdjustmentById = async (id: string) => {
    const adj = await prisma.stockAdjustment.findUnique({ where: { id }, include: { product: true, user: true } });
...
export const deleteStockAdjustment = async (id: string, isAdmin: boolean) => {
    if (!isAdmin) throw new AppError(UNAUTHORIZED, "Only admin can delete adjustments");
