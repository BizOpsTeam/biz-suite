...
function forecastNext(data: number[], horizon: number, method: string): number[] {
  if (data.length === 0) return Array(horizon).fill(0);
  if (method === 'moving-average') {
    const avg = data.slice(-horizon).reduce((a, b) => a + b, 0) / Math.min(horizon, data.length);
    return Array(horizon).fill(avg);
  } else if (method === 'linear') {
    const { slope, intercept } = linearRegression(data);
    return Array.from({ length: horizon }, (_, i) => slope * (data.length + i) + intercept);
  } else {
...
export async function getTopProducts(options: AnalyticsOptions = {}) {
  ...
  const productMap: Record<string, { product: any; totalSold: number; totalRevenue: number; timesSold: number }> = {};
...
export async function getSalesOverTime(options: AnalyticsOptions = {}) {
  ...
  return Object.entries(groupBy).map(([period, totalAmount]) => ({ period, totalAmount }));
}
...
export async function getSalesByChannel(options: AnalyticsOptions = {}) {
  ...
  for (const sale of sales) {
    channelMap[sale.channel] = (channelMap[sale.channel] || 0) + sale.totalAmount;
  }
  return Object.entries(channelMap).map(([channel, totalAmount]) => ({ channel, totalAmount }));
}
...
export async function getSalesByPaymentMethod(options: AnalyticsOptions = {}) {
  ...
  for (const sale of sales) {
    methodMap[sale.paymentMethod] = (methodMap[sale.paymentMethod] || 0) + sale.totalAmount;
  }
  return Object.entries(methodMap).map(([paymentMethod, totalAmount]) => ({ paymentMethod, totalAmount }));
}
...
export async function getTopCustomers(options: AnalyticsOptions = {}) {
  ...
  const customerMap: Record<string, { customer: any; totalSpent: number; salesCount: number }> = {};
  for (const sale of sales) {
...
export async function getAverageOrderValue(options: AnalyticsOptions = {}) {
  ...
  return { averageOrderValue: count ? totalAmount / count : 0, totalOrders: count, totalAmount };
}
...
export async function getStockouts(options: AnalyticsOptions = {}) {
  ...
  const stockouts = products.map(product => {
    ...
    return {
      ...
      name: product.name,
      stock: product.stock,
      lastSold: product.saleItems.length ? product.saleItems[product.saleItems.length - 1].createdAt : null,
      totalSold,
      estimatedLostSales: product.stock === 0 ? Math.round(totalSold * 0.1) : 0, // crude estimate
      product,
    };
  });
  return stockouts;
...
export async function getSlowMovingInventory(options: AnalyticsOptions & { threshold?: number } = {}) {
...
export async function getSalesForecast({ period = 'month', horizon = 3, method = 'auto' }: { period?: string; horizon?: number; method?: string }) {
  ...
  const lastPeriod = sales.length > 0 ? sales[sales.length - 1].period : undefined;
...
export async function getProductSalesForecast({ period = 'month', horizon = 3, method = 'auto', productId }: { period?: string; horizon?: number; method?: string; productId?: string }) {
  // Aggregate sales by period for each product
  let products: any[] = [];
  if (productId) {
    products = [await prisma.product.findUnique({ where: { id: productId } })].filter(Boolean);
  } else {
  ...
  const results = await Promise.all(products.map(async (product) => {
    ...
    return {
      productId: product.id,
      name: product.name,
      forecast: forecast.map((value, i) => ({ period: `future+${i + 1}`, forecast: value })),
      history: Object.entries(periodMap).map(([period, value]) => ({ period, value })),
      product,
    };
  }));
  return results;
...
export async function getStockoutForecast({ period = 'month', horizon = 3, method = 'auto' }: { period?: string; horizon?: number; method?: string }) {
...
export async function getSeasonality({ period = 'month', productId }: { period?: string; productId?: string }) {
  ...
  if (productId) {
    ...
    sales = saleItems.map(si => ({ createdAt: si.sale.createdAt, amount: si.price * si.quantity }));
  } else {
    sales = await prisma.sale.findMany();
    sales = sales.map(s => ({ createdAt: s.createdAt, amount: s.totalAmount }));
  }
